// src/components/MyRefrigerator.js

import React, { useState, useEffect, useCallback } from "react";
import { useAuth } from "../context/AuthContext";
import useApi from "../hook/useApi";
import "../css/Myfridge.css";
import MyRecipes from "./Myrecipes"; // ✅ 유지
...
function MyRefrigerator() {
  ...
  const fetchIngredient = useCallback(async () => {
    ...
    try {
      ...
      console.log("API Response:", response); // 💡 이 부분을 추가하여 확인
      ...
      <div className="refrigerator-list-container">
        ...
        <div className="ingredient-list-wrapper">
          <div className="ingredient-header">
            <p style={{ margin: 0 }}>
              냉장고 재료를 추가/수정하고, 선택하여 조리법을 분석해 보세요.
            </p>
          </div>
                  ...
                  {editingId !== item.id ? (
                    <>
                      <div className="ingredient-info">
                        <span className="ingredient-name-col">{item.name}</span>
