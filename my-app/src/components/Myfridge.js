...
function MyRefrigerator() {
  ...
  // 💡 수정: 서버 요청 및 기존 작동 코드에 맞춰 'name'으로 통일
  ...
  const fetchIngredient = useCallback(async () => {
    ...
    try {
      ...
      const mappedData = rawData.map((item) => ({
        id: item.id,
        name: item.ingreName, // <-- ingreName 필드의 값을 name 필드로 설정
        quantity: item.quantity,
      }));
      setIngredient(mappedData); // 매핑된 데이터를 상태에 저장
              ...
              <div className="add-form-fields">
                <div className="form-group-inline">
                  ...
                  <input
                    ...
                    id="add-name"
                    name="name" // 💡 name으로 수정
                    value={formData.name} // 💡 formData.name으로 수정
